...
if __name__ == "__main__":
    ...
    hyper = {
        ...
        'black_box_model_type': black_box.dynamics_network,
        }

    if load_model:
        with open(f"data/network_models/network_black_box_{hyper['dataset']}_seed_{seed}.jax", 'rb') as f:
            data = pickle.load(f)
    ...
    if params is None:
        params = black_box_model_fn.init(init_key, q[0:1], qd[0:1], qdd[0:1], tau[0:1])
    ...
    logs = {
        'n_batch': 1,
        'loss': jnp.array(0.0),
        'forward_mean': jnp.array(0.0), 'forward_var': jnp.array(0.0),
    ...
    # Save the Model:
    if save_model:
        with open(f"data/network_models/network_black_box_{hyper['dataset']}_seed_{seed}.jax", "wb") as file:
            pickle.dump(
                {"epoch": epoch_i,
                 "hyper": hyper,
                 "params": params,
    ...
    delan_g = black_box_dynamics_model(params, None, q, zeros, zeros, zeros)[1]
    delan_c = black_box_dynamics_model(params, None, q, qd, zeros, zeros)[1] - delan_g
    delan_m = black_box_dynamics_model(params, None, q, zeros, qdd, zeros)[1] - delan_g

    t0_evaluation = time.perf_counter()
    delan_tau = black_box_dynamics_model(params, None, q, qd, qdd, 0.0 * q)[1]
